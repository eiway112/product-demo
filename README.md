...
给任意产品/项目生成**单文件、离线自包含、可交互**的对外演示网页（图片 base64 内联、`http` 外链为 0，离线双击可用、可直接发微信——这是产出物的必要属性，任何单文件 HTML 都能做到，并非本技能独占）。
...
- **CI 首跑已回读（2026-09-20，`e6a0ae1`）**：`.github/workflows/ci.yml` 覆盖
  ...
  **已闭合（2026-09-20 23:16 回读）**：run 35518654946（`4f1f9be`）**六条腿全绿**——
  windows-latest 两条腿由「第 4 步就红、其后 16 步 skipped」变为 **20 步全绿**，
  其中「确认浏览器可用」与三条浏览器判据（含截图）均 success：v1.8.3 新增的
  Windows 浏览器覆盖在 runner 上真的跑起来了。macOS 两条腿按设计只跳过浏览器那 4 步。
  （本仓的老规矩：不把「配置已提交」当成「已经验证」——这次是 runner 自己给的结论。）
